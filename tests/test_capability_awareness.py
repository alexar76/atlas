...
def test_layer_labels_five_locales():
    for key in ("fire", "radiation", "jamming", "traffic", "events", "spacewx",
                "lightning", "alerts", "flood", "effis", "volcano", "ais", "tsunami",
                "cyclone", "adsb"):
...
def test_analyst_surfaces_brief_auto():
    ...
    assert "gaia.ais.public.read@v1" in brief
    assert "gaia.tsunami.read@v1" in brief
    assert "gaia.cyclone.read@v1" in brief
    assert "gaia.adsb.public.read@v1" in brief
    assert "atlas.watchbox.check@v1" in brief
    assert "atlas.situation.brief@v1" in brief
    assert "ais" in brief and "tsunami" in brief
    assert "cyclone" in brief and "adsb" in brief
