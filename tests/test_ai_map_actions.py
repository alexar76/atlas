...
@pytest.mark.parametrize(
    "question",
    [
        "show me what atlas is",   # "atl" → Atlanta. The product's own name.
        "show me the site",        # "sit" → a geomagnetic observatory in Alaska.
        "show the latest news",    # "new" → one in New Mexico.
        "show me the deduction",   # "ded" → one in Antarctica.
    ],
)
def test_a_three_letter_code_does_not_match_inside_a_longer_word(question):
    """The alias table's short Latin entries are codes — atl, sit, new, jfk, brw.

    Leading-boundary-only matching is right for a NAME, which inflects ("каир" →
    «в Каире», "bern" → "Berne"), and wrong for a code, which never does. Every
    time a new batch of airport/observatory aliases lands, leaving it wrong
    reintroduces the "la" in disp**la**y bug under a new three letters — most
    embarrassingly on the word ATLAS itself.
    """
    assert resolve_map_actions(question) == []


def test_a_code_still_matches_when_it_is_the_word():
    """Both boundaries, not "never matches"."""
    assert map_action_station_ids(resolve_map_actions("show ATL weather")) == ["nws-katl"]


def test_a_short_cyrillic_name_still_matches_inflected():
    """«Рим» is three characters and a NAME — it declines, so it keeps the loose rule."""
    ids = map_action_station_ids(resolve_map_actions("покажи погоду в Риме"))
    assert any("rome" in sid or "roma" in sid for sid in ids), ids
