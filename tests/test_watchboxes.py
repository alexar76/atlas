...
def test_store_create_check_delete(tmp_path: Path):
    ...
    # Unsubscribe keeps the row (and therefore its evidence log) reachable…
    assert store.unsubscribe("wb-test-01")["active"] is False
    assert store.get("wb-test-01") is not None
    # …purge is the explicit removal.
    assert store.purge("wb-test-01")
