...
@pytest.mark.asyncio
async def test_fresh_station_is_budgeted(client, monkeypatch):
    monkeypatch.setattr(main_mod, "force_limiter", RateLimiter(1))
    ok = await client.get("/api/v1/stations/om-wx-01?fresh=1")
    blocked = await client.get("/api/v1/stations/om-wx-01?fresh=1")
    assert ok.status_code == 200
    assert blocked.status_code == 429


@pytest.mark.asyncio
async def test_full_refresh_is_operator_only_not_merely_budgeted(client, monkeypatch):
    """One call re-reads the whole catalog through GAIA's pacer, so a per-IP
    budget would still let an anonymous caller queue minutes of upstream work in
    front of every interactive viewport."""
    monkeypatch.setattr(main_mod, "force_limiter", RateLimiter(50))
    monkeypatch.setattr(main_mod, "settings", Settings(operator_token="s3cret"))

    anonymous = await client.post("/api/v1/refresh")
    assert anonymous.status_code == 401
    assert "operator token" in anonymous.json()["detail"]
...
@pytest.mark.asyncio
async def test_full_refresh_fails_closed_with_no_token_configured(client, monkeypatch):
    monkeypatch.setattr(main_mod, "force_limiter", RateLimiter(50))
    monkeypatch.setattr(main_mod, "settings", Settings(operator_token=""))
    blocked = await client.post("/api/v1/refresh", headers={"X-ATLAS-Token": "anything"})
...
async def test_operator_token_skips_the_budget(client, monkeypatch):
    ...
    for _ in range(3):
        r = await client.post(
            "/api/v1/viewport", json=body, headers={"X-ATLAS-Token": "s3cret"}
        )
        assert r.status_code == 200
    # A wrong token is simply not an operator: it falls back to the per-IP
    # budget instead of being trusted. The operator's three calls never touched
    # that budget, so the first anonymous force still gets its single allowance.
    wrong = await client.post("/api/v1/viewport", json=body, headers={"X-ATLAS-Token": "nope"})
    again = await client.post("/api/v1/viewport", json=body, headers={"X-ATLAS-Token": "nope"})
    assert wrong.status_code == 200
    assert again.status_code == 429

    # …and the operator path itself still opens the refresh route.
    ok = await client.post("/api/v1/refresh", headers={"X-ATLAS-Token": "s3cret"})
    assert ok.status_code == 200
