...
# ── The ledger has to survive a redeploy ──────────────────────────────────────
#
# `consume()` fails OPEN on a write error, deliberately: a broken meter must not take
# the mesh down. That is right per request and wrong as a state to run in — the node
# reports `meter_error` into a log nobody reads and serves every priced call free.
# The default path is relative, so inside the container it lands in an image layer and
# every `up -d --build` hands each caller a fresh free tier. Checked at boot, because
# the request path is the one that forgives it.


def _metered_prod(monkeypatch):
    monkeypatch.setenv("ATLAS_PAYMENT_ENFORCED", "1")
    monkeypatch.setenv("AIFACTORY_PROD", "1")


def test_relative_default_path_refuses_to_boot(monkeypatch):
    _metered_prod(monkeypatch)
    monkeypatch.delenv("ATLAS_TRIAL_DB_PATH", raising=False)
    with pytest.raises(RuntimeError) as exc:
        payment_gate.assert_durable_ledger()
    assert "relative" in str(exc.value)


def test_path_inside_the_app_tree_refuses_to_boot(monkeypatch):
    """Absolute is not the same as durable: /app/data is still an image layer."""
    _metered_prod(monkeypatch)
    app_root = Path(payment_gate.__file__).resolve().parent
    monkeypatch.setenv("ATLAS_TRIAL_DB_PATH", str(app_root / "data" / "trials.db"))
    with pytest.raises(RuntimeError) as exc:
        payment_gate.assert_durable_ledger()
    assert "image layer" in str(exc.value)


def test_unwritable_directory_refuses_to_boot(monkeypatch, tmp_path):
    _metered_prod(monkeypatch)
    locked = tmp_path / "locked"
    locked.mkdir()
    locked.chmod(0o500)
    monkeypatch.setenv("ATLAS_TRIAL_DB_PATH", str(locked / "trials.db"))
    try:
        with pytest.raises(RuntimeError) as exc:
            payment_gate.assert_durable_ledger()
...
def test_volume_path_boots(monkeypatch, tmp_path):
    """What atlas/docker-compose.yml actually configures must pass."""
    _metered_prod(monkeypatch)
    monkeypatch.setenv("ATLAS_TRIAL_DB_PATH", str(tmp_path / "atlas_trials.db"))
    payment_gate.assert_durable_ledger()
    assert payment_gate.durability_issue() == ""


@pytest.mark.parametrize(
    "env",
    [
        {"ATLAS_PAYMENT_ENFORCED": "0", "AIFACTORY_PROD": "1"},
        {"ATLAS_PAYMENT_ENFORCED": "1", "AIFACTORY_PROD": "0"},
        {"ATLAS_PAYMENT_ENFORCED": "0", "AIFACTORY_PROD": "0"},
    ],
)
def test_unmetered_or_dev_hosts_still_boot_on_the_default(monkeypatch, env):
    """A node serving everything free has no meter worth protecting."""
