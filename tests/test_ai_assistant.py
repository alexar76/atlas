...
@pytest.mark.asyncio
async def test_a_fleet_that_will_not_warm_still_gets_an_answer(
    aggregator: Aggregator, monkeypatch
):
    """The warm is an optimisation and must never become the caller's wait.

    Measured on the live node: `ensure_all_readings` walks ~700 pins through GAIA's
    72-per-minute pacer against a five-minute reading TTL, so it cannot finish inside a
    request — and because the warm runs BEFORE the "is a provider configured" branch,
    `POST /api/ai/ask` returned nothing at all for 180 seconds on a warm, healthy node.
    Here the warm never finishes at all; the answer still has to arrive from cache.
    """
    monkeypatch.setattr(ai, "aggregator", aggregator)
    monkeypatch.setattr(ai, "any_provider_configured", lambda: False)
    monkeypatch.setattr(aggregator.settings, "analyst_warm_budget_s", 0.5)

    async def never_finishes(*_a, **_kw):
        await asyncio.sleep(120)

    monkeypatch.setattr(aggregator, "ensure_all_readings", never_finishes)

    started = time.monotonic()
    out = await ai.ask(question="What is the fleet doing?", locale="en")
    elapsed = time.monotonic() - started

    assert out["answer"], "the analyst returned nothing when the warm stalled"
    assert elapsed < 15, f"answering waited {elapsed:.1f}s on a warm that never finishes"
