...
(function (global) {
  ...
  function mount(opts) {
    ...
    panel.innerHTML =
      ...
      `<div><strong>ATLAS Analyst</strong><div class="ai-sub">LLM · sensors + map control</div></div>` +
      ...
      `<form class="ai-form">` +
      `<textarea class="ai-input" rows="2" placeholder="Ask · show Berlin · open om-wx-01 · ecosystem…"></textarea>` +
    ...
    async function ask(question, report) {
      ...
      try {
        ...
        if (!r.ok) {
          ...
          if (meta.offline) {
            answer += "\n\n— offline mode (configure a provider key or local Ollama/LM Studio)";
          } else if (meta.blocked) {
            answer += "\n\n— blocked by prompt firewall";
          } else if (meta.model) {
