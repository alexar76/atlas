...
(function (global) {
  ...
  /* Basemap providers.
   *
   * CARTO is a KEYED provider: every tile it serves without an api_key has
   * "API KEY REQUIRED — carto.com/basemaps?apikey" burned into the image, so
   * attaching it keyless stamps that watermark across the whole map. Hard rule:
   * a keyed provider is attached ONLY when a key is actually configured —
   * otherwise ATLAS uses a keyless provider and says why on the basemap button
   * in the left panel. Never on the map.
   */
  const KEYLESS_PROVIDER = "openfreemap";
  // Keyless vector styles (own glyphs + sprites, no signup, no api_key).
  const OFM_STYLE = {
    dark: "https://tiles.openfreemap.org/styles/dark",
    light: "https://tiles.openfreemap.org/styles/positron",
  };
  const PROVIDERS = {
    openfreemap: { label: "OpenFreeMap", keyed: false, attribution: OFM_ATTR },
    carto: { label: "CARTO", keyed: true, attribution: CARTO_ATTR },
    none: { label: "Off", keyed: false, attribution: "" },
  };

  let basemapCfg = null;

  /** Server-injected basemap config; the un-substituted default is keyless. */
  function readBasemapConfig() {
    let raw = {};
    try {
      const node = document.getElementById("atlas-basemap-config");
      const text = node && node.textContent && node.textContent.trim();
      if (text) raw = JSON.parse(text) || {};
    } catch (_) { /* malformed → keyless default */ }
    const key = String(raw.key || "").trim();
    let asked = String(raw.provider || "auto").trim().toLowerCase();
    if (!asked) asked = "auto";
    if (asked !== "auto" && !PROVIDERS[asked]) asked = "auto";
    let provider = asked === "auto" ? (key ? "carto" : KEYLESS_PROVIDER) : asked;
    // The whole point: a keyed provider without a key is never attached.
    const keyMissing = !!PROVIDERS[provider] && PROVIDERS[provider].keyed && !key;
    if (keyMissing) provider = KEYLESS_PROVIDER;
    return { provider, key, keyMissing, requested: asked };
  }

  function basemapConfig() {
    if (!basemapCfg) basemapCfg = readBasemapConfig();
    return basemapCfg;
  }

  /** What the HUD/panel needs to name the active basemap (and why). */
  function basemapInfo() {
    const cfg = basemapConfig();
    const meta = PROVIDERS[cfg.provider] || PROVIDERS[KEYLESS_PROVIDER];
    return {
      provider: cfg.provider,
      label: meta.label,
      attribution: meta.attribution,
      keyed: !!meta.keyed,
      keyMissing: cfg.keyMissing,
    };
  }

  function cartoTiles(kind, key) {
    const path = kind === "light" ? "light_all" : "dark_all";
    const q = key ? `?api_key=${encodeURIComponent(key)}` : "";
    return ["a", "b", "c"].map(
      (host) => `https://${host}.basemaps.cartocdn.com/${path}/{z}/{x}/{y}@2x.png${q}`
    );
  }
  ...
  /** Keyless Sentinel-2 mosaic — the globe's "physical" surface. */
  function physicalGlobeStyle(uiKind) {
    return {
      version: 8,
      projection: { type: "globe" },
      glyphs: GLYPHS,
      sources: { earth: globeEarthSource() },
      layers: [
        {
          id: "earth-void",
          type: "background",
      ...
      metadata: { "atlas:surface": "physical", "atlas:basemap": uiKind },
    };
  }

  /** CARTO raster — keyed only. Callers must pass a non-empty key. */
  function cartoStyle(uiKind, proj, key) {
    ...
    return {
      ...
      layers: [
        {
          id: globe ? "carto-countries" : "carto",
          type: "raster",
          source: "carto",
          paint: countriesRasterPaint(uiKind),
        },
      ],
      sky: skyForBasemap(uiKind, proj),
      light: lightForProjection(proj, uiKind),
      metadata: { "atlas:surface": "countries", "atlas:basemap": uiKind },
    };
  }

  /** No tiles at all (provider "none") — pins still render over the void. */
  function blankStyle(uiKind, proj) {
    return {
      version: 8,
      projection: { type: proj === "globe" ? "globe" : "mercator" },
      glyphs: GLYPHS,
      sources: {},
      layers: [
        {
          ...
          type: "background",
          paint: {
            "background-color": uiKind === "light" ? "#dbe4ee" : "#040b12",
          },
        },
      ],
      sky: skyForBasemap(uiKind, proj),
      light: lightForProjection(proj, uiKind),
      metadata: { "atlas:surface": "countries", "atlas:basemap": "none" },
    };
  }
   ...
   * @returns {object|string} MapLibre style, or a style URL for the keyless
   *   vector provider (a remote style carries none of our sky/light/projection,
   *   so those are re-applied on style.load).
   */
  function styleForBasemap(kind, projection, surface) {
    ...
    if (globe && surf === "physical") return physicalGlobeStyle(uiKind);
    if (cfg.provider === "carto" && cfg.key) return cartoStyle(uiKind, proj, cfg.key);
    if (cfg.provider === "none") return blankStyle(uiKind, proj);
    return OFM_STYLE[uiKind === "light" ? "light" : "dark"];
  }
  ...
  function isUnsetEventPin(s) {
    ...
    if (!(layer === "quake" || layer === "fire" || layer === "jamming" || layer === "traffic"
        ...
        || layer === "cyclone" || layer === "adsb" || layer === "smoke"
        || layer === "precipitation" || layer === "radar" || layer === "sea_ice")) {
      return false;
    }
  ...
  /* The keyless vector styles are grayscale "dark matter" / positron: the dark
     one paints land rgb(12,12,12) and water rgb(27,27,29) and never uses the
     Natural Earth source it ships, so from globe zoom the planet is a black
     ball with floating labels. Retint it into the ATLAS palette and put the
     relief back under the water. Every write is guarded — a provider may
     rename or drop a layer, and a missing one must not break the map. */
  const NE_RELIEF_TILES =
    "https://tiles.openfreemap.org/natural_earth/ne2sr/{z}/{x}/{y}.png";
  const NE_ATTR = '<a href="https://www.naturalearthdata.com/">Natural Earth</a>';
  const DARK_TINT = [
    ["background", "background-color", "#0b1720"],
    ["water", "fill-color", "#061d2a"],
    ["waterway", "line-color", "#0c2734"],
    ["water_name", "text-color", "#4c8496"],
    ["water_name", "text-halo-color", "rgba(2,8,12,0.85)"],
    ["landcover_ice_shelf", "fill-color", "#15242c"],
    ["landcover_glacier", "fill-color", "#16262e"],
    ["landcover_wood", "fill-color", "#0f2019"],
    ["landuse_park", "fill-color", "#0f2019"],
    ["landuse_residential", "fill-color", "#111d24"],
    ["building", "fill-color", "#0f1b22"],
    ["building", "fill-outline-color", "#16252d"],
    ["highway_minor", "line-color", "#16242c"],
    ["highway_major_subtle", "line-color", "#1d2f38"],
    ["highway_major_inner", "line-color", "#14222a"],
    ["highway_motorway_subtle", "line-color", "#1a2b34"],
    ["railway", "line-color", "#1e2f38"],
    ["railway_minor", "line-color", "#1a2a32"],
    ["railway_transit", "line-color", "#1a2a32"],
    ["boundary_state", "line-color", "rgba(125,160,190,0.30)"],
    ["boundary_country_z0-4", "line-color", "rgba(61,214,198,0.45)"],
    ["boundary_country_z5-", "line-color", "rgba(61,214,198,0.40)"],
  ];
  const DARK_LABELS = [
  ...
  function tuneVectorBasemap(map, basemapKind) {
    if (!map || typeof map.getLayer !== "function") return;
    try {
      // Already tuned for this style. Also the loop guard: every write below
      // fires `styledata`, which is what re-invokes this.
      if (map.getLayer("atlas-relief")) return;
    } catch (_) { return; }
    const light = basemapKind === "light";
    const setPaint = (id, prop, value) => {
      try {
        if (map.getLayer(id)) map.setPaintProperty(id, prop, value);
      } catch (_) { /* provider renamed a layer — skip it */ }
    };

    // Relief goes under the ocean fill: land gets landforms, water stays water.
    try {
      if (!map.getSource("atlas-relief")) {
        map.addSource("atlas-relief", {
          type: "raster",
          tiles: [NE_RELIEF_TILES],
          tileSize: 256,
          minzoom: 0,
          // The Natural Earth set ends here; without a cap MapLibre would ask
          // for city-zoom tiles that do not exist.
          maxzoom: 6,
          attribution: NE_ATTR,
        });
      }
      if (!map.getLayer("atlas-relief") && map.getLayer("water")) {
        map.addLayer(
          {
            id: "atlas-relief",
            type: "raster",
            ...
            paint: {
              ...
              "raster-brightness-min": light ? 0.34 : 0.05,
              "raster-brightness-max": light ? 1 : 0.82,
              "raster-saturation": light ? -0.12 : 0.2,
              "raster-contrast": light ? 0.06 : 0.22,
            },
          },
          "water"
        );
      }
    } catch (_) { /* style settling — the next style.load retries */ }

    if (light) return; // positron already reads at every zoom
    DARK_TINT.forEach(([id, prop, value]) => setPaint(id, prop, value));
    DARK_LABELS.forEach((id) => {
      setPaint(id, "text-color", "#9dc0b6");
      setPaint(id, "text-halo-color", "rgba(2,8,12,0.85)");
    });
  }
