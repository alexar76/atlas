...
(function (global) {
  ...
  function mount(opts) {
    ...
    function syncQuietAttribution() {
      ...
      if (!parts.length) {
        // Remote vector styles carry no per-source attribution — credit the
        // active provider, never a provider we did not attach.
        const attr = basemapInfo ? basemapInfo().attribution : "";
    ...
    function reloadMapStyle() {
      ...
      map.once("style.load", () => {
        ...
        try {
          ...
          tuneVectorBasemap(map, basemapKind);
          if (typeof map.setProjection === "function") {
    ...
    /* Basemap provenance lives in the panel, not on the map: a keyed provider
       attached without its key answers every tile with "API KEY REQUIRED"
       printed across the image. ATLAS keeps the paid layer off and says so
       here — one button, left panel. */
    function mountBasemapNote() {
      const foot = document.querySelector("#panel .panel-foot");
      if (!foot || !basemapInfo || el("basemap-btn")) return;
      const wrap = document.createElement("div");
      wrap.className = "basemap-wrap";
      const btn = document.createElement("button");
      btn.type = "button";
      btn.id = "basemap-btn";
      btn.className = "basemap-btn";
      btn.setAttribute("aria-expanded", "false");
      btn.setAttribute("aria-controls", "basemap-note");
      const note = document.createElement("div");
      note.id = "basemap-note";
      note.className = "basemap-note";
      note.hidden = true;

      const info = basemapInfo();
      const state = info.provider === "none"
        ? tr("basemap.off")
        : info.keyed
          ? tr("basemap.keyed")
          : tr("basemap.keyless");
      btn.innerHTML =
        ...
        `<span class="label">${esc(tr("basemap.title"))}: ${esc(info.label)}</span>`;
      btn.title = `${tr("basemap.title")}: ${info.label} — ${state}`;
      const lines = [`<strong>${esc(info.label)}</strong> · ${esc(state)}`];
      if (info.keyMissing) lines.push(esc(tr("basemap.keyMissing")));
      if (info.attribution) lines.push(info.attribution);
      note.innerHTML = lines.map((line) => `<p>${line}</p>`).join("");
    ...
    function markMapReady() {
      ...
      try {
        // A keyless vector style is remote: it ships mercator and none of our
        // sky/light, so the projection has to be re-asserted after it loads.
        if (typeof map.setProjection === "function") {
          map.setProjection({ type: projectionMode === "globe" ? "globe" : "mercator" });
        }
      } catch (_) { /* style settling */ }
    ...
    // A remote style's layers may not be parsed at the first styledata, and
    // markMapReady only ever runs once — so keep retrying the retint until it
    // takes (it self-guards once the relief layer is in).
    map.on("styledata", () => {
      try {
        tuneVectorBasemap(map, basemapKind);
      } catch (_) { /* style settling */ }
