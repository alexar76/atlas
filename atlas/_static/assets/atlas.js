  ...
  function mount(opts) {
    ...
    function syncQuietAttribution() {
      ...
      if (!box) {
        ...
        box.setAttribute("aria-label", "Map attribution");
    ...
    function playGlobeIntro() {
      ...
      setTimeout(() => {
        if (projectionMode === "globe" && !userInteracting) startSpinSoon(0);
      }, 3600);
    }
    ...
    function setProjectionToggleUi() {
      ...
      btn.title = globe ? "Switch to flat map" : "Switch to 3D globe";
      const label = btn.querySelector(".label");
      if (label) label.textContent = globe ? "Globe" : "Flat";
    ...
    function setSurfaceToggleUi() {
      ...
      btn.title = physical
        ? "Switch to countries map (labels)"
        : "Switch to physical satellite map";
      const label = btn.querySelector(".label");
      if (label) label.textContent = physical ? "Physical" : "Countries";
      try {
        document.documentElement.dataset.surface = surfaceMode;
      } catch (_) { /* ignore */ }
    }
    ...
    if (global.AtlasTheme && mode === "full") {
      global.AtlasTheme.mountPicker(el("hud"), { onBasemap: applyBasemap });
    }
    ...
    function setHud(snap) {
      ...
      if (sq) {
        ...
        sq.textContent = fn > 1 ? `${qn} quakes · ${fn} fires` : `${qn} quakes`;
      }
    ...
    function layerLabel(info, key) {
      const loc = (typeof opts.getLocale === "function" ? opts.getLocale() : "en") || "en";
      const short = String(loc).slice(0, 2).toLowerCase();
    ...
    function renderLayers(snap) {
      ...
      const counts = {};
      // Honest totals from API summary (global FIRMS / Safecast clusters / …).
      // Do NOT count slim catalog stations — that shows fire=1 forever.
      ...
      if (Number.isFinite(fires) && fires > (counts.fire || 0)) {
        counts.fire = fires;
      }
      // Parent SKUs carry hotspot_matched even when summary briefly lags after restart.
      for (const s of snap?.stations || []) {
        if (!s || !s.layer) continue;
        const matched = Number(s.hotspot_matched);
      ...
      for (const [key, info] of Object.entries(meta)) {
        ...
        row.innerHTML =
          ...
          `<span class="count">${Number(counts[key] || 0)}</span>`;
    ...
    function renderStations(snap) {
      ...
      for (const s of list) {
        ...
        const badge = s.live
          ? `<span class="badge live" title="Public-API relay with provenance source">LIVE</span>`
          : s.mode === "sim"
            ? `<span class="badge sim" title="Physics simulator — no upstream source">SIM</span>`
            : s.has_reading
              ? `<span class="badge">reading</span>`
              : `<span class="badge">pin</span>`;
        const modeHint = s.live ? "LIVE relay" : (s.mode === "sim" ? "SIM" : "pin");
        // Upstream (GAIA relay) strings are untrusted — escape every sink.
        card.innerHTML =
          `<div class="row"><span class="id">${esc(s.id)}</span>${badge}</div>` +
          `<div class="row" style="margin-top:4px"><span class="headline">${esc(s.headline || "Tap for reading")}</span></div>` +
    ...
    async function refreshViewport(force) {
      ...
      if (cached) {
        ...
        if (hint) {
          ...
          hint.textContent = `${n} in view · cache hit`;
      ...
      try {
        ...
        if (hint) {
          ...
          let fireBit = "";
          if (fires) {
            fireBit = ` · ${firePins} fire in view / ${fires} total`;
          }
          hint.textContent = `${n} in view · ${cachedN} cached · ${hits} ttl${fireBit}`;
          hintStickyUntil = Date.now() + 4000;
        }
      } catch (err) {
