...
(function (global) {
  ...
  function mountPicker(hudEl, opts) {
    ...
    wrap.innerHTML =
      `<button type="button" class="theme-btn" id="theme-btn" aria-haspopup="listbox" aria-expanded="false" title="Theme">` +
      `<span class="dot" aria-hidden="true"></span><span class="label">Theme</span>` +
      `</button>` +
    ...
    function renderMenu() {
      ...
      menu.innerHTML = THEMES.map((t) => {
        ...
        return (
          ...
          `<span class="meta"><span class="name">${t.name}</span><span class="kind">${t.kind}</span></span>` +
          `</button>`
        );
      }).join("");
