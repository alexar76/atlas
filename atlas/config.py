...
class Settings(BaseSettings):
    ...
    # Basemap tiles.
    # "auto" = keyed provider when a key is configured, keyless otherwise.
    # Explicit values: "openfreemap" (keyless vector), "carto" (needs a key),
    # "none" (no tiles — sensors over a flat background).
    # A keyed provider is NEVER attached without its key: keyless CARTO tiles
    # arrive with "API KEY REQUIRED" burned into the image, which would paint
    # that watermark across the whole map.
    ...
    # Bbox-scoped USGS water-quality clusters cannot share the ordinary
    # device_id cache: two simultaneous cameras may ask for different sites.
    water_quality_viewport_ttl_s: float = 60.0
    water_quality_viewport_cache_size: int = 128
    water_quality_history_path: str = "data/water_quality_history.json"
    water_quality_history_limit: int = 96
    # Click detail may force-refresh if older than this.
    detail_fresh_s: float = 20.0
    # A pin click must answer, not spin. The live read keeps running into the
    # cache when it outruns this, and the card says it is refreshing — a cold
    # station behind a slow upstream used to hold the panel for gaia_timeout_s
    # (180s in production) on the very first click.
    detail_budget_s: float = 8.0

    # Lazy prefetch: pad viewport (degrees) then warm remaining catalog in background.
    # Fleet is hundreds of pins (anchors + 50-city Open-Meteo mesh ×2) — an in-process
    ...
    # How long the Analyst may spend warming sensors before it answers from cache. The warm
    # walks the catalog through GAIA's per-minute pacer, so on the live fleet (~700 pins, a
    # 5-minute reading TTL) it cannot finish inside a request: /api/ai/ask returned nothing at
    # all for 180s+. The warm is an optimisation — the prompt is built from whatever is
    # cached — so it gets a budget rather than the caller's patience.
    analyst_warm_budget_s: float = 8.0
    ...
    # Wide viewport: stratified grid sampling instead of global brightest top-N.
    firms_stratified_min_span_deg: float = 25.0
    firms_stratified_grid_deg: float = 5.0
    ...
    # Hub federation signing (Ed25519 + optional ML-DSA-65 via ORACLE_PQC/ATLAS_PQC).
    # Prefer ATLAS_SIGNING_SEED_B64 for classical seed in prod; PQ key is file-only.
