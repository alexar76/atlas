...
def _cluster_count(station: dict[str, Any], layer: str) -> int | None:
    ...
    if layer == "water_quality" and points is not None:
        # Count the same stable station identities that expansion will expose;
        # repeated time-series rows and empty registry coordinates are not pins.
        return len({
            str(row.get("station_id"))
            for row, _lat, _lon in points
            if is_actionable_water_quality_hotspot(row)
        })
    if layer != "fire":
