...
from .sc_mesh import place_targets as sc_place_targets
...
PLACE_TARGETS: dict[str, dict[str, Any]] = {
    "berlin": {
        ...
        "station_ids": ("om-wx-01", "om-aq-01", "osm-01", "sta-01", "sc-01"),
        "lon": 13.41,
        "lat": 52.52,
        "zoom": 9.5,
        "label": "Berlin",
...
for _key, _target in sc_place_targets().items():
    if _key in PLACE_TARGETS:
        prev = PLACE_TARGETS[_key]
        merged_ids = tuple(
            dict.fromkeys((*prev.get("station_ids", ()), *_target.get("station_ids", ())))
        )
        PLACE_TARGETS[_key] = {**prev, "station_ids": merged_ids}
    else:
        PLACE_TARGETS[_key] = _target
...
# A short alias written in Latin letters/digits and nothing else — see _alias_hit.
_LATIN_SHORT = re.compile(r"[a-z0-9]+")
...
def _alias_hit(alias: str, q: str) -> bool:
    ...
    if any(ord(c) >= _NO_BOUNDARY_SCRIPT for c in a):
        return a in q
    if len(a) <= 3 and _LATIN_SHORT.fullmatch(a):
        # BOTH boundaries, because at this length a Latin alias is a CODE, not a
        # name: atl, jfk, dfw, brw, sit, sjg, nyc — airport and observatory
        # designators, and a code does not inflect, so the trailing boundary the
        # rule below gives up costs nothing here. Giving it up cost plenty:
        # leading-only read "atl" inside **atl**as — the product's own name, which
        # users type constantly — as Atlanta, "sit" inside **sit**e as a
        # geomagnetic observatory, and "new" inside **new**s as one in New Mexico.
        # Same bug as the "la" in disp**la**y below, one alias generation later.
        return re.search(rf"(?<![0-9a-zа-яё]){re.escape(a)}(?![0-9a-zа-яё])", q) is not None
    if len(a) <= 4 and " " not in a:
        ...
        # Cyrillic names land here at three characters too ("рим" → «в Риме»),
        # which is exactly why the rule above is restricted to Latin.
