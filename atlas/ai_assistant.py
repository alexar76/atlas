...
def _warm_budget() -> float:
    return max(0.5, float(getattr(aggregator.settings, "analyst_warm_budget_s", 8.0) or 8.0))


async def _warm_within_budget(coro: Any, what: str) -> None:
    """Warm what fits in the budget, then answer from cache.

    The full-fleet warm walks the catalog through GAIA's 72-per-minute pacer. On the live node
    (707 pins, a five-minute reading TTL) that cannot finish inside a request, and because it
    sits BEFORE the "is a provider configured" branch, ``POST /api/ai/ask`` returned nothing at
    all — measured 180s with no response, on a warm, healthy node. The warm only improves the
    grounding; ``build_live_context`` reads whatever is cached, so running out of budget is a
    log line and a slightly staler answer rather than a request that never ends.
    """
    budget = _warm_budget()
    try:
        ...
        log.warning(
            "analyst: %s did not warm within %.1fs — answering from cache", what, budget
        )
    except Exception:  # noqa: BLE001 — a cold upstream must not fail the answer
        log.exception("analyst: warming %s failed — answering from cache", what)
...
async def ask(
    *,
    ...
    if aggregator.settings.analyst_warm_all:
        ...
        await _warm_within_budget(
            aggregator.ensure_all_readings(force=False), "the cached fleet"
        )
    elif warm_ids:
        await _warm_within_budget(
            aggregator._ensure_readings(warm_ids[:12], force=False), "the named stations"
        )
    if warm_ids:
        if len(warm_ids) == 1 or action_ids:
            primary = action_ids[0] if action_ids else warm_ids[0]
            try:
                focus_detail = await asyncio.wait_for(
                    aggregator.station_detail(primary, fresh=False), timeout=_warm_budget()
                )
            except Exception:
                focus_detail = None
