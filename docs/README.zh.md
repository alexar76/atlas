...
**文档：** [EN](GUIDE.md) · [RU](i18n/GUIDE.ru.md) · [ES](i18n/GUIDE.es.md) · [FR](i18n/GUIDE.fr.md) · [ZH](i18n/GUIDE.zh.md) · **[地图图例](i18n/LEGEND.zh.md)**
...
付费能力会被计量（`ATLAS_PAYMENT_ENFORCED=1`），抵达它们有两条路。匿名调用方有免费额度 — 每小时
五次 invoke，按 `X-AIMarket-Sandbox-Visitor` 或其地址计。需要更多的一方持有**预付额度账户**，把
密钥放在 `X-API-Key` 里：按公布价格逐次扣费、不受免费额度限制、被拒绝时不计费。付费响应会带上
`X-Atlas-Credit-Charged-Usd` / `-Balance-Usd`；当余额降到 `ATLAS_CREDITS_LOW_BALANCE_USD`
（默认 $1.00）及以下时，还会带 `-Low: 1`。

完整说明，五种语言：[**CREDIT-ACCOUNTS**](i18n/CREDIT-ACCOUNTS.zh.md) ·
[EN](CREDIT-ACCOUNTS.md) · [RU](i18n/CREDIT-ACCOUNTS.ru.md) · [ES](i18n/CREDIT-ACCOUNTS.es.md) ·
[FR](i18n/CREDIT-ACCOUNTS.fr.md)。
